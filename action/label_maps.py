...
NTU120_LABELS = NTU60_LABELS + NTU120_EXTRA_LABELS


def get_label_map(name: str) -> list[str]:
    normalized = str(name).strip().lower()
    if normalized == "ntu60":
        return NTU60_LABELS
    if normalized == "ntu120":
        return NTU120_LABELS
    raise ValueError(f"Unsupported label map: {name}")
