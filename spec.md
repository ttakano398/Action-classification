...
本書は、`RTMO` による人物・骨格推定結果を入力として、`BlockGCN` で人物ごとの動作ラベルを分類する PoC システムの仕様を定義する。
...
本システムでは、`BlockGCN` が担当する分類対象と、後段ルールベースで扱うイベントを分離する。
...
`映像入力 -> RTMO -> 軽量 ID 割当 -> track ごとの時系列バッファ -> BlockGCN -> ラベル平滑化 -> JSON/可視化出力`
